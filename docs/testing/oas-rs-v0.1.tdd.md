...
| OpenAPI hot path | `OAS_BENCH_ITERATIONS=1000000 cargo bench --bench router -- --nocapture` | OpenAPI/Swagger enabled and disabled apps both measured 4 allocations/op; the single release sample showed -0.01% enabled-vs-disabled time delta, which is supporting evidence rather than a statistical gate. |
